...
2. `/admin`에 접속합니다.
3. 이 작업 폴더의 `.env`에 있는 `ADMIN_SETUP_TOKEN` 값을 **초기 설정 키**에 입력합니다. 같은 값이 배포 환경에도 비밀값으로 설정되어 있습니다.
...
Sites의 기본 배포는 소유자 전용입니다. 실제 커뮤니티에서 사용하려면 사이트 접근 대상을 커뮤니티에 맞게 별도로 변경해야 합니다. 앱의 닉네임/비밀번호 로그인과 Sites의 방문 허용 범위는 별개입니다.
